...
router.patch('/:userId', verifyToken, [
    body('username').if(body('username').exists()).trim().isLength({min: 1}).escape().withMessage("Username must be specified.").isAlphanumeric().withMessage("Username has non-alphanumeric characters."),
    body('first_name').if(body('first_name').exists()).trim().isLength({min: 1}).escape().withMessage("First name must be specified.").isAlphanumeric().withMessage("First name has non-alphanumeric characters."),
    body('last_name').if(body('last_name').exists()).trim().isLength({min: 1}).escape().withMessage("Last name must be specified.").isAlphanumeric().withMessage("Last name has non-alphanumeric characters."),
    body('old_password').if(body('old_password').exists()).trim().isLength({min: 8}).escape().withMessage("You must enter your old password."),
    body('password').if(body('password').exists()).trim().isLength({min: 8}).escape().withMessage("Password must have 8 or more characters."),
    body('confirm_password').if(body('confirm_password').exists()).trim().escape().custom((value, {req}) => value === req.body.password).withMessage("The passwords do not match."),
    body('admin_passcode').if(body('admin_passcode').exists()).trim().escape().custom((value, {req}) => value === process.env.ADMIN_PASSCODE).withMessage("The admin passcode you entered is incorrect."),
], (req, res) => {
...
router.delete('/:userId', verifyToken, (req, res) => {
    jwt.verify(req.token, process.env.JWT_SECRET_KEY, async(err, token) => {
        ...
        try {
            ...
            await Post.deleteMany({ "user._id": new ObjectId(req.params.userId) }); // Delete any posts associated with the deleted user
            await Comment.deleteMany({ "user._id": new ObjectId(req.params.userId) }); // Delete any comments associated with the deleted user
